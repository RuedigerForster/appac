...
plot_global_fit <- function (data = NA, fit.coefs = NA,
  ...
  subtitle <- "" # paste0("Standard error of the residuals = ", format(round(rse, digits = 2), nsmall = 2), ".")
...
plot_local_fit <- function (data, sample, peak, coefs, colors, show.compensated.areas = TRUE, plot.residuals = TRUE, bins = 50) {
  # if (!sample %in% names(data@correction@samples))
  #   stop("Unknown sample '", sample, "'. Sample may be any of: ", paste0("'", names(data@correction@samples), sep = "'"))
  # if (!peak %in% colnames(data@correction@samples[[sample]]$raw.area))
  #   stop("Sample '", sample, "' does not include peak '", peak, "'. Peak may be any of: ", paste0("'", colnames(data@correction@samples[[sample]]$raw.area), sep = "'"))
  ...
  draw_function <- function(x) return(aref + slop*(x - P_ref))
  p1 <- ggplot (X) +
    # raw areas
    geom_point(data = na.omit(X), mapping = aes(x = x, y = y1),
  ...
  if (show.compensated.areas) {
    p1 <- p1 + geom_point(data = X, mapping = aes(x = x, y = y2),
                        color = highlight_color, # factor(s)),
                        size = 1,
                        shape = 20,
                        alpha = 0.5)
  }
  p1 <- p1 + theme(text = element_text(size=14), legend.position = "none") +
    labs(title = title, subtitle = subtitle, x = xlab, y = ylab) +
    ylim(l.y.limit, h.y.limit) +
    # stat_smooth(method = "lm",
    #             data = X, mapping = aes(x = x, y = y2),
    #             formula = y ~ x,
    #             level = 0.9999, color = line_color,
    #             fill = fill_color, alpha = 1) # 0.5
  ...
  # (sample, data@correction, type = "raw")[, peak]
  ...
  # drift + bias compensated areas
  if (show.compensated.areas) {
    p2 <- p2 + geom_point(data = Z, mapping = aes(x = x, y = y2),
                          color = highlight_color, # factor(s)),
                          size = 1,
                          shape = 20,
                          alpha = 0.5)
  }
  p2 <- p2 + theme(text = element_text(size=14), legend.position = "none") +
  ...
  if (show.compensated.areas) {
    p3 <- p3 +
      geom_histogram(aes(x = y2, y = after_stat(density)), binwidth = binwidth, position="identity", fill = highlight_color, alpha = 0.4) # color = highlight_color,
  }
  p3 <- p3 +
...
plot_control_chart <- function (data, sample, peak, colors, show.compensated.areas = TRUE, plot.residuals = TRUE, bins = 50) {
  # browser()
  # if (!sample %in% names(data@correction@samples))
  #   stop("Unknown sample '", sample, "'. Sample may be any of: ", paste0("'", names(data@correction@samples), sep = "'"))
  # if (!peak %in% colnames(data@correction@samples[[sample]]$raw.area))
  ...
  p3 <- p3 + geom_vline(aes(xintercept = 0), color = line_color, linetype="dashed", linewidth = 1) +
    geom_function(color = line_color, linewidth = 1, fun = dnorm, args = list(mean = mean(y1), sd = sd(y1)))
