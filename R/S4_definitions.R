...
setMethod("plotControlChart",
  ...
  function(object, sample, peak, colors) {
    ...
    if (!(peak %in% colnames(object@correction@samples[[sample]]$raw.area))) {
      # cat("I was here _o_")
      stop("Sample '", sample, "' does not include peak '", peak, "'. Peak may be any of: ", 
