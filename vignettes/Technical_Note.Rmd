...
drift which causes bias in the data, the code negotiates the experimental errors 
...
with a linear modelm to obtain the slope $dY/dP$ of the fitted line and the expected 
...
Autocorrelation can be visually identified in a plot of a local fit. It may be 
identified by a heavy-tailed or skewed distribution of the residuals; in extreme 
cases several modes will appear, see Fig. 1.
...
$$dY/dP \sim 0 + Y_{ref} + Y_{ref}^2 \tag{10}$$ 
...
$$ Y  \sim  \overline{Y}_{ref} (+ \overline{Y}_{ref}^2) \tag{13}$$
...
Now $Y_{raw,comp}$ is subjected to a pressure fit according to eq. (9) and (10).
Using the newly obtained value of $\kappa$, the raw areas are corrected to eq. (7) 
again and the drift is recalculated according to eq. (13) and (14). Repeatedly, 
corrected and compensated corrected are calculated with raw areas as the input.

After the value of $\kappa$ has been improved in this way, it is now time to polish
the drift. ...
