---
title: "APPAC User Guide"
author: "Rüdiger Forster"
description: |
    Short introduction into the appac package.
output:
...
## Introduction
This document illustrates the usage of the appac package and also explains how to obtain your own data that can be processed with appac. For a comprehensive overview of the APPAC method and its applications, please refer to [1].

The appac package features a main function, which is 

`appac (data, P.ref, appac.control,  appac.colnames)`

It is surrounded by several other functions (which are actually methods) that help to retrieve the input and the fitted data.

The package uses the S4 class system. To familiarize yourself with S4 classes and methods, we recommend the section ‘The S4 object system’ from Hadley Wickham's excellent book ‘Advanced R’, which is fortunately available online [2].
...
Note:
The user must decide in advance which peaks/components are to be evaluated; only these peaks may be included in the input data frame, or the data frame must be filtered accordingly before it is passed to the appac function. 
...
Note:
As the sample and peak names of the input `data` are used internally as variable names, they must comply with the R conventions for variable names. If they do not comply with the conventions, these names are converted and a respective warning is issued. To be able to use one of these names in a method, the changed names must be used. (Maybe it is a good idea to provide a conversion function `checkNames` which does this conversion on occasion of a user call instead of doing it inside of a black box. Your opinions, please.)
...
appac.control <- list(min.data.points = 100)
...
## define the colors list
colors <- list(highlight_color="red", lowlight_color="darkgrey", 
               line_color="darkblue", fill_color="lightblue")

## get the sample names
samples <- names(x@correction@samples)

for (s in samples) {
  ## get the peak names of sample s
  peaks <- colnames(x@correction@samples[[s]]$raw.area)
  gridExtra::grid.arrange(grobs = lapply(peaks, function(p) suppressWarnings(plotLocalFit(object=x, sample=s, peak=p, colors=colors))), ncol=2)
}
```
...
## define the colors list
colors <- list(highlight_color="yellow", lowlight_color="darkgrey", 
               line_color="darkblue", fill_color="lightblue")
...
for (s in samples) {
  ## get the peak names of sample s
  peaks <- colnames(x@correction@samples[[s]]$raw.area)
  gridExtra::grid.arrange(grobs = lapply(peaks, function(p) suppressWarnings(plotControlChart(object=x, sample=s, peak=p, colors=colors))), ncol=2)
}
```
...
## Some internals of the appac function

## Set up your own control experiments

### Control samples
Control samples must meet the following requirements:
